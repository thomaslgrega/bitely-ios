...
for desc_payload in \
  ...
  "a PR URL for another repository|$(jq -n '{tool_response:{stdout:"https://github.com/thomaslgrega/bitelyapi/pull/50\n"}}')" \
  "an MCP PR opened in another repository|$(mcp_payload 50 thomaslgrega bitelyapi)"
do
  setup
...
# --- MCP fallback --------------------------------------------------------------
setup
run_hook "$(mcp_payload 7 thomaslgrega bitely-ios)"
